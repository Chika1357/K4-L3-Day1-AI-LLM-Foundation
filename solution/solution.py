...
def compare_models(prompt: str) -> dict:
    ...
    num_words = len(gpt4o_response.split())
    tokens_est = num_words / 0.75
    gpt4o_cost_estimate = (tokens_est / 1000.0) * PRICING_PER_1K_TOKENS["gpt-4o"]["output"]
    return {
...
def run_assistant(
    ...
    while True:
        ...
        turn_cost = estimate_cost(user_msg, reply_text, model=OPENAI_MODEL)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    """
    header = f"{'Prompt':<30} | {'GPT-4o Response':<30} | {'Mini Response':<30} | {'4o Latency':<10} | {'Mini Latency':<10}"
    separator = "-" * len(header)
    rows = [header, separator]
    for r in results:
        p = (r.get("prompt", "")[:27] + "...") if len(r.get("prompt", "")) > 30 else r.get("prompt", "")
        r4 = (r.get("gpt4o_response", "")[:27] + "...") if len(r.get("gpt4o_response", "")) > 30 else r.get("gpt4o_response", "")
        rm = (r.get("mini_response", "")[:27] + "...") if len(r.get("mini_response", "")) > 30 else r.get("mini_response", "")
        l4 = f"{r.get('gpt4o_latency', 0.0):.2f}s"
        lm = f"{r.get('mini_latency', 0.0):.2f}s"
        rows.append(f"{p:<30} | {r4:<30} | {rm:<30} | {l4:<10} | {lm:<10}")
    return "\n".join(rows)
